...
async function scanUrl(url) {
  ...
  try {
    ...
    // Block heavy resources that aren't needed for accessibility checks
    ...
    // Navigate — use domcontentloaded first, then wait for network to settle
    await page.goto(url, { timeout: 45000, waitUntil: "domcontentloaded" });

    // Wait for network to mostly settle (max 10s extra)
    try {
      await page.waitForNetworkIdle({ idleTime: 1000, timeout: 10000 });
    } catch (_) {
      // Network didn't fully settle — that's fine, proceed anyway
    }
